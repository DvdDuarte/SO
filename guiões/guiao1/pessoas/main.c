...
int main(int argc, char const *argv[]) {
	...
	switch(*(argv[1] + 1)) {
		...
		case 'i':
		    ...
		    struct stat st;
		    fstat(fd, &st);
		    off_t filezise = st.st_size;
		    int pos = (int)filezise / sizeof(struct person);

		    if(!no_output) printf("Insertion successfull - registo %d\n", pos);
		    close(fd);
		    break;
		...
		case 'u':
			...
			while(read(fd,&person, sizeof(struct person)) > 0){
				if(registry == -1 && strcmp(person.name,argv[2] == 0) || registry == i) {
					...
					if(!no_output) puts("Update successfull");
					goto UPDATE_SUCCESS;
				}
				i++;
			}
